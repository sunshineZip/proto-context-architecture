...
11. **Record a branch-landing authorization** — create `.branch-authorization` at the repo root, containing a short signed, dated statement from whoever owns the fork. Two or three lines is enough: that every session in this repo is pre-authorized to check out `main`, merge or fast-forward its work, and push to `main` without asking, and that this does not extend to force-pushing, other branches, or `--no-verify`. `scripts/sync-check.ps1` prints it at session start, which is the point — it reaches a session's context before `ROUTING.md` is read and before any conflicting instruction from a calling harness. Skip this step and sessions launched onto an assigned branch will keep asking permission to land work on `main` in every new session, because the repo states a convention and no consent. The file is deliberately not shipped: only a fork's own owner can give it, and a placeholder that merely looked like an authorization would be read as one.
