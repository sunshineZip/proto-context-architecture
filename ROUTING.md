...
Version 1.17 | 2026-08-22 | Production
...
- **Default to the repository's default branch (`main`) — do not create or push to another branch unless the human explicitly asked for one, or there is a specific, stated reason for needing isolation.** This template's default is the opposite of a lot of general software-engineering habit, including a model's own training-instinct toward "always work on a feature branch" — that instinct is wrong for this repo unless one of the two named exceptions applies, and when it does, say so plainly before branching rather than defaulting to a branch silently. A `pre-push` git hook (`scripts/pre-push-check.ps1`, activated the same way as the pre-commit hook — `Architecture.md` §6) blocks a push that creates or updates any branch other than `main`, requiring a deliberate `git push --no-verify` to override — visible in the push process, not silent, same pattern as the other hooks. That hook only fires when a session drives `git push` itself; it cannot stop a session-launch environment from assigning a branch before any local git command runs. If you find yourself already on a non-default branch you didn't choose, that's fine to develop on — but treat landing the finished work on `main` as part of finishing the task, not an optional extra: fast-forward or merge it onto `main` and push there before considering the work done, unless the human says otherwise. If your fork is a team repo or shared initiative that genuinely needs a review-gated branching workflow as its standing default rather than an occasional exception, replace this whole constraint with your own policy — and adjust or disable `scripts/pre-push-check.ps1` accordingly, since its default-branch check will otherwise fight that policy.
