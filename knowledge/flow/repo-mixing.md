...
Apply this at the point of pulling content across, not afterwards. Mixing into another of your own forks is a lighter-stakes version of handing content to an outside recipient — content still leaves the repo it was written for — not a stakes-free one, because two forks can have genuinely different downstream audiences.

- **`[SENSITIVE]`** (`MarkdownConventions.md` §8) — pull across only the specific claim the task actually needs, never the whole section it sits in. Lean toward less. A follow-up question is always available; a fact already restated into another fork's permanent, append-only session-log is not easily un-said. If the task appears to need a tagged claim whose loss of context would matter — anything a reader could act on directly, such as a credential, an account identifier, a medical detail, or a third party's private circumstances — stop and name that specific claim to the human rather than deciding it either way yourself.
- **Untagged content** — not automatically safe. An untagged claim has not been reviewed and cleared; it has simply never been assessed. Apply the same judgment as if it carried a tag, and default to leaving it where it is.
- **A guest fork with no sensitivity tagging at all** (many forks won't have any) — fall back to the base `ROUTING.md` Hard Constraint every fork already has: pause and ask before writing a secret, credential, or confidential detail into any tracked file. That gate does not go away just because the content is moving fork-to-fork rather than person-to-outsider.

**Do not read an Executive Summary as pre-cleared.** `MarkdownConventions.md` §8 requires tagged content to sit in a named section rather than in the sections `ROUTING.md` Step 4 loads by default, which makes "just take the Executive Summary" a tempting shortcut for a cross-fork pull. It is not a substitute for this gate: the rule is unenforceable mechanically, a guest fork may predate it, and the one real violation observed carried no tag at all because that fork classified sensitivity per section rather than per claim.

**If a guest fork has adopted a stronger tier than `[SENSITIVE]`** — some do, marking a subset that must never leave the repo under any circumstances — that fork's own tier governs what crosses, and nothing at that tier is pulled by default. Read that fork's own conventions for what its tier means; this document does not define one, and a fork's local extension is not something home may reinterpret.
...
Once a mixing session has pulled anything durable into home (per §8), record that it happened — not the content itself — in home's `projects/system/mixing-log.md`. One row per **(session, guest, destination)** that produced a durable write, appended chronologically, never edited (same append-only spirit as `session-log.md`, though this file isn't subject to the turn-protocol machinery — a plain running table). Columns: date, guest repo, domains/sections drawn from, the home project or domain the content landed in, and a one-line note on any `[SENSITIVE]` content included, or content that was flagged and declined.
...
3. Pull across only what the stated purpose needs (§7) — the leave policy's practical effect on drop-off/pickup, not an unrelated performance-review narrative sitting in the same domain.
...
3. §7 applies as written to any fork using `MarkdownConventions.md` §8's `[SENSITIVE]` signal — it reuses that tagging as-is, and nothing further is needed. If a fork you'll mix has no sensitivity tagging at all, decide now whether to add it or to rely on the base secret/confidential-detail Hard Constraint as the fallback (§7, third bullet). If a fork has extended §8 with a stronger tier of its own, note that too: §7's last paragraph defers to that fork's definition rather than reinterpreting it.
