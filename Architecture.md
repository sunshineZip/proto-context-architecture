...
[repo-name]/
  ...
  library/                            ← Cross-domain reference-work registry — see authoring-guidelines.md §9.2
    reference-index.md                ← Registry of every reference work ever touched, stored or not
...
- **`library/`** — cross-domain reference-work registry (`reference-index.md`) and, for cornerstone-status works only, the physical files themselves, stored directly in `library/`. See `knowledge/domains/authoring-guidelines.md` §9.2–9.3.
...
11. **Record a branch-landing authorization** — `.branch-authorization` at the repo root holds a short signed, dated statement from whoever owns the fork. **If you forked this template, delete the inherited file first and write your own.** It is tracked, so a fork inherits the *template owner's* consent, which is not consent for your repo and is not theirs to give for it. Two or three lines is enough: that every session in this repo is pre-authorized to check out `main`, merge or fast-forward its work, and push to `main` without asking, and that this does not extend to force-pushing, other branches, or `--no-verify`. `scripts/sync-check.ps1` prints it at session start, which is the point — it reaches a session's context before `ROUTING.md` is read and before any conflicting instruction from a calling harness. Delete it and record nothing, and sessions launched onto an assigned branch will keep asking permission to land work on `main` every time, because the repo then states a convention and no consent.
12. **(Optional) Set up the restricted-tier companion repo** — only if this fork will systematically handle real third-party sensitive/restricted material (personal records, confidential documents, regulated data) at volume, and only before ingesting the first real source file. See `knowledge/flow/restricted-tier.md`. Most forks won't need this — occasional sensitive content is already covered by `ROUTING.md`'s pause-and-ask Hard Constraint and the pre-commit secret-pattern check.
