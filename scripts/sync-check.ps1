...
# (knowledge/flow/git-collaboration.md section 2-3): that one catches staleness
...
# --- Locate git rather than assuming it is on PATH. PATH first, so Linux containers and
#     any normal Git for Windows install work unchanged; then GitHub Desktop's bundled
#     copy, which on some Windows machines is the only git present. Without this the
#     bare calls below produced CommandNotFound, $repoRoot came back empty, and this
#     script exited 0 having checked nothing -- the silent-inertness failure it exists
...
if (-not $gitExe) {
    Write-Host "sync-check: no git executable found -- this session's checkout was NOT verified against origin. Anything read from it may be stale." -ForegroundColor Red
    exit 0
}
...
& $gitExe -C $repoRoot fetch origin $branch 2>&1 | Out-Null
...
$remoteShowOutput = & $gitExe -C $repoRoot remote show origin 2>$null
...
& $gitExe -C $repoRoot rev-parse --verify "origin/$branch" 2>&1 *> $null
if ($LASTEXITCODE -ne 0) {
    exit 0
}

$hasLocalChanges = [bool](& $gitExe -C $repoRoot status --porcelain 2>$null)
$behindCount = [int](& $gitExe -C $repoRoot rev-list "HEAD..origin/$branch" --count 2>$null)
$aheadCount = [int](& $gitExe -C $repoRoot rev-list "origin/$branch..HEAD" --count 2>$null)
...
if ($behindCount -gt 0 -and $aheadCount -eq 0 -and -not $hasLocalChanges) {
    $incoming = @(& $gitExe -C $repoRoot log --oneline "HEAD..origin/$branch" 2>$null)
    & $gitExe -C $repoRoot merge --ff-only "origin/$branch" 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) {
...
if ($behindCount -gt 0 -and $aheadCount -gt 0) {
    ...
    Write-Host "Do not push as-is. See knowledge/flow/git-collaboration.md section 3-4 for the rebase" -ForegroundColor Yellow
