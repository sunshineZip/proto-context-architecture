...
**Proactive classification, every time:** storing a source is not something a session does only when asked, and not something it does only once per session. Every document a human uploads, quotes, or forwards is its own classification event — is this legal/financial/administrative documentation (an official confirmation letter, a policy, a contract), or merely informational? Run this classification before responding to whatever surface-level question came attached to the document, not instead of it. This applies identically to the first document a fresh session receives and to the fifth one arriving forty turns into an otherwise unrelated conversation. Classification is never silent — state which way a document was classified even when the answer is "not a source": a human who never hears "no" has no way to notice a session stopped checking. See `ROUTING.md` Standing Rules for the session-wide instinct this codifies the mechanics for.

> **Worked negative example (the failure this guards against):** A session correctly classifies and stores a document uploaded early in a conversation. Several turns later, after a long unrelated back-and-forth on a different topic, the human uploads a second qualifying document — an official letter from a third party — attached to a question about what the letter means, not about storage. The session answers the question well, but never re-runs the classification, and never states whether the document was stored either way. The gap only surfaces because the human notices the silence and asks directly why it wasn't stored. **What should have happened instead:** answer the question, and in the same turn, separately state the classification outcome regardless — "this also reads as an official confirmation letter, storing it under `sources/`" or "this one is informational, not storing it as a source" — every time, regardless of what else the turn is about or how far into the conversation it arrives.

There is no mechanical backstop for this the way there is for the structural-changes-logging pre-commit hook (`ROUTING.md` Hard Constraints): a git hook can only inspect what actually gets committed, and a document that was never classified leaves no file-level trace for a hook to check against. The constraint is enforceable only by the model re-running the instinct on every qualifying upload — treat that as a documented limitation, not a solved problem.
